...
def Bellman_Con(M,V0):
   ...
   while i<p:
        j=0
        while X[i,j] <= X[i,j+1] and j<p-2:
            X[i,j+2] = M[i,j+2] + beta*V0[j+2] #We deffine matrix X
            j = j+1
        i = i+1 
   argmax_i = np.argmax(X,axis=1) #We deffine a vector that delivers the maximun of each row
...
V_con, g_con, iterations = solution(Bellman_Con)

stop = timeit.default_timer()
time_con = stop - start
print('Time - Concavity: '+ str(time_con))
print('Number of iterations - concavity:'+str(iterations))
...
plt.plot(k, g_con, label = 'policy function')
