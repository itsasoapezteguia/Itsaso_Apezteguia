...
"""b) Iterations of the value function taking into account monotonicity of the optimal decision
rule."""
...
def Bellman_Mon(M,V0):
   ...
   while i<p:
        j=0
        while j+t<p:
            X[i,j+t] = M[i,j+t] + beta*V0[j+t] #We deffine matrix X
            j = j+1   
...
V_mon, g_mon, iterations = solution(Bellman_Mon)

stop = timeit.default_timer()
time_mon = stop - start
print('Time - Monotonicity: '+str(time_mon))
print('Number of iterations - monotonicity:'+str(iterations))
...
plt.plot(k, g_mon, label = 'policy function')
