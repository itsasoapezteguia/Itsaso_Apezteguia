...
def Bellman(M,V0):
   ...
   while i<p:
        j=0
        while j<p:
            X[i,j] = M[i,j] + beta*V0[j] #We deffine matrix X
            j = j+1
        i = i+1 
...
V, g, iterations = solution(Bellman)

stop = timeit.default_timer()
time_bruteForce = stop - start
print('Time - brute force: '+str(time_bruteForce))
print('Number of iterations - brute force:'+str(iterations))
...
plt.plot(k, g, label = 'policy function')
